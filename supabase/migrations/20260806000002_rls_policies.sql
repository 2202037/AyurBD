...
begin
  -- Only client traffic is guarded. Inside a SECURITY DEFINER function
  -- current_user is the function owner, not 'authenticated', so the
  -- trigger-driven internal paths from schema.sql PART 3.4 (notably
  -- payments_apply_verification setting appointments.payment_status) pass
  -- straight through. This is the check that keeps the guard from
  -- breaking the very logic it protects.
...
create trigger aa_guard_doctors
  ...
  for each row execute function
    public.guard_admin_only_columns('verification_status', 'status', 'rating', 'total_reviews', 'rejection_reason', 'is_deleted');
...
-- Verification is the doctor's action (/provider/doctor/payments/verify).
-- The patient gets no UPDATE at all: a submitted payment is evidence, and
-- letting the submitter edit it afterwards would defeat verification.
create policy payments_update_doctor
...
-- The customer may cancel and edit delivery details; totals and
-- order_number are frozen by aa_guard_orders.
