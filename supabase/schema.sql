...
-- payments.payment_status â€” a different set from payment_state.
...
-- ---------------------------------------------------------------------
-- users â€” profile mirror of auth.users.
--
...
create table public.doctors (
  ...
  -- Platform commission, set by the admin when a provider is verified: this
  -- percentage of every paid appointment fee is retained by the platform and
  -- the remainder is the doctor's share (see payments_apply_verification).
  ...
  constraint doctors_commission_check check (commission_percentage >= 0 and commission_percentage <= 100),
...
create table public.hospitals (
  ...
  constraint hospitals_commission_check check (commission_percentage >= 0 and commission_percentage <= 100),
...
create table public.clinics (
  ...
  constraint clinics_established_year_check
    check (established_year is null
           or established_year between 1800 and extract(year from now())::int + 1),
  constraint clinics_commission_check check (commission_percentage >= 0 and commission_percentage <= 100),
...
create table public.pharmacies (
  ...
  constraint pharmacies_commission_check check (commission_percentage >= 0 and commission_percentage <= 100),
...
create table public.payments (
  ...
  -- of the amount) and provider_share is what the provider is owed â€” the two
  -- always sum to `amount`. NULL while unverified.
  admin_share            numeric(10,2),
  provider_share         numeric(10,2),
  created_at       timestamptz not null default now(),
  updated_at       timestamptz not null default now(),
  ...
  -- Every digital payment must carry the payer's transaction reference so
  -- the admin can match it against the mobile-banking / bank statement.
  -- Cash is paid at the chamber (no reference) and sslcommerz is written
  -- by the payment provider with its own gateway_transaction_id, so both
  -- are exempt. The client already enforces this in the sheet
  -- (my_appointments_screen.dart); this makes the server refuse a
  -- reference-less bKash/Nagad/Rocket/card/bank-transfer submission too.
  constraint payments_transaction_ref_check check (
...
-- Rejected and verified payments are excluded, which lets a patient resubmit
-- after a rejection.
create unique index uq_payments_pending_appt
  on public.payments (appointment_id)
  where payment_status = 'pending';

-- ---------------------------------------------------------------------
-- payment_sessions â€” one SSLCommerz attempt per appointment.
--
...
create table public.orders (
  ...
  notes            text,
  -- Money split for pharmacy order payments: computed by
  -- orders_apply_verification when payment_status becomes 'paid', using the
  -- pharmacy's commission_percentage. admin_share is retained by the platform,
  -- provider_share is owed to the pharmacy owner.
  admin_share      numeric(10,2),
  provider_share   numeric(10,2),
  created_at       timestamptz not null default now(),
  updated_at       timestamptz not null default now(),
  ...
  constraint orders_split_check check (
    (admin_share is null and provider_share is null)
    or (admin_share is not null and provider_share is not null
...
-- ---------------------------------------------------------------------
-- provider_payouts â€” the provider's side of the money split.
--
-- When a payment or order is verified, payments_apply_verification /
-- orders_apply_verification record how much the provider is owed
-- (provider_share) here. It is a ledger, not a wallet: in the free
-- testing model the patient pays the platform's account, the platform
-- keeps admin_share, and the provider's share is paid out by the admin
-- offline (bKash/Nagad transfer). `status` tracks that offline step:
--   pending  -> owed, waiting on the admin's transfer
--   paid     -> the admin has transferred the money
--   reversed -> the underlying payment was refunded, so nothing is owed
-- Exactly one of payment_id / order_id is set (the polymorphic source).
-- ---------------------------------------------------------------------
...
comment on column public.provider_payouts.payment_id is 'Set when the payout comes from a verified appointment payment; mutually exclusive with order_id.';
comment on column public.provider_payouts.order_id is 'Set when the payout comes from a paid pharmacy order; mutually exclusive with payment_id.';

create index idx_provider_payouts_user   on public.provider_payouts (provider_user_id);
create index idx_provider_payouts_status on public.provider_payouts (status);
-- One payout per verified payment, and one per paid order.
create unique index uq_provider_payouts_payment on public.provider_payouts (payment_id) where payment_id is not null;
create unique index uq_provider_payouts_order   on public.provider_payouts (order_id)   where order_id is not null;
...
-- ---------------------------------------------------------------------
-- emergency_hotlines â€” reference data for the emergency screen.
--
...
-- ---------------------------------------------------------------------
-- emergency_sms â€” a log, not a transmitter.
--
...
-- ---------------------------------------------------------------------
-- audit_log â€” written only by triggers.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- app_audit_log â€” written by the application (login, logout, ...).
-- ---------------------------------------------------------------------
...
-- boundary, and every one pins search_path â€” an unpinned search_path on
-- a SECURITY DEFINER function is a privilege-escalation vector.
-- =====================================================================

-- ---------------------------------------------------------------------
-- set_updated_at â€” replaces MySQL's ON UPDATE current_timestamp().
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- audit_row_change â€” one generic replacement for the 36 MySQL triggers.
--
...
-- target when a review is repointed â€” something the PHP never did.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- reviews_check_target â€” the integrity check a polymorphic FK cannot do.
...
-- ---------------------------------------------------------------------
-- generate_confirmation_code â€” was provider_confirmation_code() in PHP.
-- Same 32-char alphabet (no I, O, 0, 1 â€” unreadable aloud) and length 8.
...
-- ---------------------------------------------------------------------
-- orders_set_order_number â€” was 'AYU-' + str_pad(id) in pharmacy.php.
--
...
-- ---------------------------------------------------------------------
-- notify â€” the single writer for public.notifications.
--
...
-- ---------------------------------------------------------------------
-- appointments_notify â€” was 3 insert sites:
...
-- ---------------------------------------------------------------------
-- appointments_refund_on_cancel â€” was appointments.php:603.
--
...
-- ---------------------------------------------------------------------
-- payments_notify â€” was 2 insert sites:
...
begin
  ...
  if new.payment_status is distinct from old.payment_status then
    if new.payment_status = 'verified' then
      -- NOT appended: the confirmation code does not exist yet. It is
      -- minted by appointments_set_confirmation_code only when the
      -- provider later confirms the booking (payment_verified just marks
      -- the appointment paid). Claiming it here would be a lie.
...
-- ---------------------------------------------------------------------
-- payments_apply_verification â€” was provider.php:458-482.
--
...
-- ---------------------------------------------------------------------
-- orders_apply_verification â€” the pharmacy-order twin of
-- payments_apply_verification.
--
-- Fires when orders.payment_status becomes 'paid' (the pharmacy owner
-- confirms the patient's offline payment). The pharmacy's commission is
-- read at that moment and the total is split into admin_share (platform
-- cut) and provider_share (owed to the pharmacy's business account), and
-- a pending payout is recorded.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- providers_notify â€” was admin.php:335, one function per provider table.
--
...
-- ---------------------------------------------------------------------
-- orders_notify â€” was pharmacy.php:513.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- feedback_notify â€” was admin.php:679. Fires when an admin answers.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- handle_new_user â€” mirrors auth.users into public.users on signup.
--
...
--
-- Reads name/phone/role/gender from raw_user_meta_data. role is NOT
-- taken verbatim: only the five self-registerable roles are accepted and
-- anything else — including 'admin' — clamps to 'patient'. The enum cast
-- alone is not a boundary (the user_role enum contains 'admin'), and
-- metadata is client-supplied, so without this whitelist a patched client
-- could sign up with user_metadata = {"role":"admin"} and become an admin.
-- Admin accounts are created only by supabase/admin_bootstrap.sql.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- handle_user_email_change â€” keeps public.users.email in step when the
...
-- ---------------------------------------------------------------------
-- available_slots â€” the booking calendar's slot generator.
--
...
begin
  ...
  return query
    ...
    select s.ts::time
      from slots s
     where -- never offer a slot in the past (compare wall-clock slot times
           -- against Dhaka wall-clock now; now() alone would be UTC and
           -- let a slot "just passed" in Dhaka stay bookable for up to 6h)
           s.ts > (now() at time zone 'Asia/Dhaka')
       and not exists (
             select 1
               from public.appointments a
...
-- ---------------------------------------------------------------------
-- guard_provider_insert â€” the INSERT half of the PART 1 column guards.
--
...
-- writes (and internal paths, where session_user is not authenticated or
-- anon) pass straight through.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- guard_appointments_insert â€” force the money/status fields a patient
...
-- ---------------------------------------------------------------------
-- appointments_guard_confirm — the escrow hand-off.
--
-- In the platform's money model the patient pays the PLATFORM, the admin
-- verifies the payment (keeping the commission), and only then is the
-- provider's share credited to them. Confirming the booking is therefore the
-- provider's act, and it must happen AFTER the payment is verified — not
-- before, as the PHP flow allowed. This trigger enforces that order: a
-- transition to `confirmed` while the appointment still carries a fee and
-- has not been paid is refused, and only the owning doctor (or an admin)
-- may confirm at all. Free (fee = 0) consultations stay confirmable
-- immediately.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- appointments_guard_reschedule â€” the UPDATE half of slot validation.
--
-- INSERT paths (appointments_book / guard_appointments_insert) re-check
-- the slot through available_slots(), but the RLS policy
-- appointments_update_patient lets a patient PATCH their own
-- appointment_date/appointment_time directly, and nothing re-validated
-- them. A patient could move a booking to a past slot, an off-duty day,
-- outside the published window, or (racing) onto another patient's slot.
-- This BEFORE UPDATE trigger closes that: any change to the date or time
-- must pass available_slots() again, and only a 'pending' appointment may
-- be moved at all. Admin bypasses; internal writers are unaffected.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- guard_payments_insert â€” a submitted payment is evidence, and the
...
-- ---------------------------------------------------------------------
-- payments_verified_immutable â€” a verified payment is a financial record.
--
-- Once payment_status = 'verified' the money split is minted
-- (admin_share/provider_share), the appointment is marked paid and a
-- provider payout is recorded. Rewriting amount/method/reference or
-- deleting the row would silently corrupt that ledger, so even the admin
-- (the only role with UPDATE/DELETE on payments) is blocked. The correct
-- reversal is the refund flow: cancel the appointment, which flips
-- appointments.payment_status to 'refunded' and reverses the payout
-- (appointments_refund_on_cancel). The payments row itself is terminal.
-- 'rejected' stays mutable: an admin may re-verify a rejected payment.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- guard_reviews_insert â€” the INSERT half of review integrity.
--
-- A review must start 'pending' (the old policy let a client insert
-- status = 'approved' and self-approve), must be attributed to the
-- caller, and â€” for a doctor review â€” must reference an appointment the
...
begin
  ...
  if new.reviewable_type = 'doctor' then
    ...
    -- The review must be about a consultation that has actually happened:
    -- the appointment slot is in the past and the appointment is not
    -- cancelled or expired. This is the "review after the consultation"
    -- rule from the product brief, enforced in the backend so no UI bug
    -- can leak a premature review.
    select exists (
...
-- ---------------------------------------------------------------------
-- guard_orders_insert / guard_order_items_insert â€” orders are trusted
...
-- ---------------------------------------------------------------------
-- appointments_book â€” the single, race-safe booking RPC.
--
...
-- ---------------------------------------------------------------------
-- place_order â€” the only way to create an order.
--
...
-- transaction, snapshots the line items and empties the cart â€” all
...
begin
  ...
  -- Free delivery over à§³500, otherwise à§³60 â€” the same rule the basket screen
...
-- ---------------------------------------------------------------------
-- expire_stale_appointments â€” turns past unpaid appointments 'expired'.
--
...
-- ---------------------------------------------------------------------
-- users_ban_enforce â€” what happens the moment an admin flips is_active
...
-- ---------------------------------------------------------------------
-- my_stats / doctor_stats / admin_dashboard_stats â€” server-side dashboard
-- tallies.
--
...
-- plain CREATE OR REPLACE is refused once the old version exists: drop
-- first, then recreate.
...
as $$
  select
    ...
    -- Net earnings: the provider's share of verified payments only. The
    -- commission (admin_share) is not the doctor's money.
    ...
    -- Money owed to this provider but not yet paid out by the admin.
    ...
    -- Total commission the platform kept from this provider's payments.
...
-- ---------------------------------------------------------------------
-- 3.1 updated_at â€” every table that had ON UPDATE current_timestamp().
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3.2 audit_log â€” the 12 tables MySQL audited, same coverage.
...
-- The money and commerce tables (the audit gap: provider_payouts was the
-- only place money changes hands that was never logged).
create trigger provider_payouts_audit after insert or update or delete on public.provider_payouts for each row execute function public.audit_row_change();
...
create trigger order_items_audit     after insert or update or delete on public.order_items     for each row execute function public.audit_row_change();
create trigger payment_sessions_audit after insert or update or delete on public.payment_sessions for each row execute function public.audit_row_change();
create trigger cart_audit            after insert or update or delete on public.cart            for each row execute function public.audit_row_change();
create trigger notifications_audit   after insert or update or delete on public.notifications   for each row execute function public.audit_row_change();
...
-- Confirm only after payment (escrow flow) — `aa_` so it fires before the
-- confirmation-code trigger that the status transition triggers.
create trigger aa_guard_confirm
  before update of status on public.appointments
  for each row execute function public.appointments_guard_confirm();

-- A patient may move a booking only to a slot that is still genuinely
-- free (day/window/past rules all re-checked via available_slots).
...
-- Split a paid pharmacy order into platform commission + provider share.
...
-- Verified payments are a financial record: the admin may not rewrite or
-- delete them (refund via the appointment instead).
...
-- that flag a view would silently bypass RLS â€” the single most common
...
-- ---------------------------------------------------------------------
-- doctor_specialties â€” the filter chips on the doctor list.
--
...
-- Column-level restriction on top of the row-level RLS. The public
-- directory intentionally shows a provider's name/phone/email/profile
-- image (see doctor_directory), but residence and health columns are
-- never needed publicly: address, blood_group, gender and city stay
-- invisible to the anon role, so a broad `select *` on users errors
-- instead of leaking them. Authenticated is untouched — a user must keep
...
-- expire_stale_appointments is a statement-level AFTER trigger, not an RPC:
-- no client should run the maintenance pass directly, and the trigger fires
-- it without needing the caller's EXECUTE grant. Least privilege.
...
-- The payout ledger is trigger-written (no INSERT grant needed) and
-- read/updated by the provider and the admin under RLS.
